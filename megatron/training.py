...
def get_model(model_provider_func):
    ...
    if device is not None:
        if args.no_cuda:
            device.set_is_use(False)
        else:
            device.set_is_use(True)
            curr_dev_name = device.current_device_name()

    for model_module in model:
        model_module.to(curr_dev_name)
        ...
        if args.data_efficiency_curriculum_learning and build_train_valid_test_datasets_provider is not None:
            ...
            if mpu.get_tensor_model_parallel_rank() == 0:
                ...
                if args.train_samples:
                    ...
                    device = get_accelerator()
                    if device is None or not device.is_use():
                        update_train_iters(args)
                else:
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    is_cuda = True if dev_acc is not None and dev_acc.is_use() else False
        ...
        if not skipped_iter:
            if is_cuda:
                total_loss_dict[key] = total_loss_dict.get(
                    key, device.FloatTensor([0.0])) + loss_dict[key]
            else:
                total_loss_dict[key] = total_loss_dict.get(
        ...
        if args.log_optimizer_states_to_tensorboard and optimizer is not None:
            ...
            if device is not None and device.is_use():
                if args.zero_stage > 0:
                    ...
                    opt_stats = device.FloatTensor(opt_stats)
                    torch.distributed.all_reduce(opt_stats, group=mpu.get_data_parallel_group())
                    opt_stats_2 = device.FloatTensor(opt_stats_2)
    ...
    if iteration % args.log_interval == 0:
        ...
        for key in total_loss_dict:
            if key not in [advanced_iters_key, skipped_iters_key,
                ...
                total_loss_dict[key] = device.FloatTensor([0.0]) \
                    if device is not None else torch.FloatTensor([0.0])
            ...
            if mpu.is_pipeline_last_stage(ignore_virtual=True):
                ...
                for loss_dict in loss_dicts:
                    for key in loss_dict:
                        if 'moe' not in key:
                            device = get_accelerator()
                            zero = device.FloatTensor([0.0]) if device is not \
                                and device.is_use() None else torch.FloatTensor([0.0])
                            total_loss_dict[key] = total_loss_dict.get(
                                key, zero) + loss_dict[key]
...
def build_train_valid_test_data_iterators(
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        if device is not None and device.is_use():
            flags = device.LongTensor([int(do_train), int(do_valid), int(do_test)])
        else:
            flags = torch.LongTensor([int(do_train), int(do_valid), int(do_test)])
    else:
        if device is not None and device.is_use():
            flags = device.LongTensor([0, 0, 0])
        else:
